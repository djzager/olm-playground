...
operator_b_name = 'example-operator-b'
operator_b_namespace = name + '-' + operator_b_name
operator_b_subscription = {
    'name': operator_b_name,
    'namespace': operator_b_namespace,
    'channel': 'alpha',
    'catalog_source': name
}
...
def run():
    ...
    common.create_namespace(name=operator_b_namespace)
    common.create_operator_group(name=operator_a_name, namespace=operator_a_namespace)
    common.create_operator_group(name=operator_b_name, namespace=operator_b_namespace)
    common.create_catalog_source(name=name, image=v1image, display_name=name)

    # Subscribe to operator b and wait for it to be installed
    common.create_subscription(**operator_b_subscription)
    install_plan_name = common.wait_ip_on_subscription(name=operator_b_name, namespace=operator_b_namespace)
    common.wait_ip_complete(name=install_plan_name, namespace=operator_b_namespace)
    # Show the current status of olm objects
    common.dump_olm_bits(namespace=operator_b_namespace, install_plan_name=install_plan_name)
...
def teardown():
    ...
    common.delete_subscription(name=operator_b_name, namespace=operator_b_namespace)
    ...
    common.delete_operator_group(name=operator_b_name, namespace=operator_b_namespace)

    # Remove the namespaces
    common.delete_namespace(name=operator_a_namespace)
    common.delete_namespace(name=operator_b_namespace)
